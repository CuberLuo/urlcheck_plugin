...
Dialog.prototype = {
  _init: function () {
    ...
    this.$bodyHeight =
      document.documentElement.clientHeight || document.body.clientHeight //可见区高度
    this.show()
  },
  show: function () {
    this.createDialog()
  ...
  createDialog(width, height) {
    ...
    var bg = document.createElement('dialog_bg')
    ...
    bt.style.background =
      'linear-gradient(141deg,#40E0D0 0%,#00FFFF 51%,#40E0D0 75%)'
    ...
    bt.onmouseover = function () {
      ...
      bt.style.background =
        'linear-gradient(141deg,#40E0D0 30%,#00FFFF 30%,#40E0D0 30%)'
    }
    //鼠标离开事件
  ...
  enter(el, done) {
    ...
    const runkeyframes =
      `@keyframes ball-run{
          ...
          60%{
              top: ` +
      (parseInt(el.style.top.replace('px', '')) + 50) +
      `px;
          }
          100%{
              top: ` +
